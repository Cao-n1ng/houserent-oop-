...
public class HouseService {
    ...
    //这里的业务逻辑怎么写好？是一个return还是统一出口？
    public boolean write(int id, String name, String phone, String address, double rentcost, String state) {
        if (name != "-1") house[id].setName(name);
        if (phone != "-1") house[id].setPhone(phone);
        if (address != "-1") house[id].setAddress(address);
        if (rentcost != -1) house[id].setRentcost(rentcost);
        if (state != "-1") house[id].setState(state);
        return true;
